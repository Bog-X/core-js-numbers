...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstV = Math.atan2(y1, x1);
  const secondV = Math.atan2(y2, x2);
  const angle = secondV - firstV;
  return Math.abs(((angle * 180) / Math.PI) * (Math.PI / 180));
}
...
function getCountOfOddNumbers(number) {
  ...
  if (number1 === 0 || Number.isNaN(number1)) {
    return 0;
  }

  for (let i = 1; i <= number1; ) {
    if (i % 2 !== 0) {
      count += 1;
    }
    i += 1;
